...
from src.core.database import save_summary_to_db, get_summaries
from src.utils.file_exporter import (
    save_as_txt, save_as_pdf, save_as_docx, 
    save_as_xlsx, save_as_image
)

def handle_summarize_command(args: argparse.Namespace):
    """Handles the summarization logic for the 'summarize' command.
    ...
    try:
        ...
        for i, source in enumerate(args.sources):
            ...
            if summary.get("error"):
                logging.error(f"Could not summarize '{source}'. Reason: {summary.get('main_summary')}")
                continue
            ...
            if args.save_to_db:
                ...
                if saver_func:
                    ...
                    logging.warning(f"Unsupported file format '{ext}'. Use .txt, .pdf, .docx, .xlsx, .png, or .jpg.")
...
def handle_query_command(args: argparse.Namespace):
    """Handles the database query logic for the 'query' command.
    ...
    try:
        ...
        for summary in summaries:
            ...
            summary_text = summary.get('summary_text', {}).get('main_summary', '[No summary text found]')
...
def main():
    """Main function to parse arguments and execute the chosen command."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    summarize_parser.add_argument("sources", nargs='+', help="One or more URLs or local file paths (PDF, DOCX, XLSX).")
    summarize_parser.add_argument("-l", "--language", default="português", help="Language of the summary (e.g., 'english'). Default: português.")
    summarize_parser.add_argument("-s", "--style", default="a concise paragraph", help="Style of the summary (e.g., 'bullet points'). Default: a concise paragraph.")
    
    output_group = summarize_parser.add_mutually_exclusive_group()
    output_group.add_argument("-o", "--output-file", help="Base filename to save the summary. Supported formats: .txt, .pdf, .docx, .xlsx, .png, .jpg.")
    output_group.add_argument("-db", "--save-to-db", action="store_true", help="Save the summary to the SQLite database.")
    ...
    query_parser.add_argument("-l", "--limit", type=int, default=5, help="Limit the number of results. Default: 5.")
    query_parser.add_argument("-u", "--url-contains", help="Filter summaries by URL containing this string.")
