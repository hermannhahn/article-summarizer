...
def summarize_text(
    model: genai.GenerativeModel, text: str, language: str, style: str
) -> dict:
    """Send text to the Gemini model for summarization.
    ...
    logging.info(
        f"Sending text to AI for summarization in {language} with style '{style}'..."
    )
    ...
    try:
        ...
        if not summary_text:
            ...
            return {
                "main_summary": "Warning: Received an empty summary.",
                "error": False,
            }
        ...
        return {
            "main_summary": f"An error occurred during summarization: {e}",
            "error": True,
        }
